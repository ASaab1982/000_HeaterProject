...
static bool readSensor(NimBLEClient* pClient) {
    ...
    if (!pClient->connect(NimBLEAddress(XIAOMI_SENSOR_MAC))) {
        D_PRINTLN(F("[Xiaomi] Connect failed"));
        return false;
    }

    // Give the sensor 1 s to complete GATT service discovery before querying
    ...
    NimBLERemoteCharacteristic* pChar = pSvc->getCharacteristic(CHAR_UUID);
    if (!pChar) {
    ...
    if (pChar->canNotify()) {
        ...
        for (int i = 0; i < 100 && !s_notifyReceived; i++) {
    ...
    pClient->disconnect();
    vTaskDelay(pdMS_TO_TICKS(500));   // Let NimBLE finish the disconnect before returning
    return gotReading;
}
